var webim = require('./webimWx');
var selToID, loginInfo, avChatRoomId, selType, selToID, selSess, selSessHeadUrl;
//监听大群新消息（普通，点赞，提示，红包）
...
function showMsg(msg) {
  var fromAccount, fromAccountNick, subType;
  ...
  switch (opType) {
    case webim.GROUP_TIP_TYPE.JOIN: //加入群
      ...
      for (var m in userIdList) {
        text += userIdList[m] + ',';
        if (userIdList.length > WEB_IM_GROUP_TIP_MAX_USER_COUNT && Number(m) === maxIndex) {
    ...
    case webim.GROUP_TIP_TYPE.MODIFY_MEMBER_INFO: //群成员资料变更(禁言时间)
      ...
      for (var m in memberInfoList) {
        ...
        if (memberInfoList.length > WEB_IM_GROUP_TIP_MAX_USER_COUNT && Number(m) == maxIndex) {
